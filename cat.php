...
<?php }?>
  ...
  <div class="w3-main" <?php if(isset($_SESSION['username'])){echo 'style="margin-left:300px;margin-top:43px;"'; } else{echo 'style="margin-top:43px;"'; } ?>>
   ...
   <?php
  $sql="SELECT * FROM product_upload WHERE product_status=1 AND category='$product_category'";
  $res=mysqli_query($con,$sql);
  if (mysqli_num_rows($res) > 0) {
    // output data of each row
    while($row = mysqli_fetch_assoc($res)) {
        echo '<div class="w3-container w3-quarter w3-padding">
                <div class="w3-card-4 w3-center">
                  ...
                  <a href="buy.php?id='.$row["product_id"].'" class="w3-btn w3-margin w3-theme-l1">Buy</a>
                  <a href="product_detail.php?id='.$row["product_id"].'" class="w3-btn w3-margin w3-theme-l1">Details</a>
                </div>
              </div>';
        }
      }
