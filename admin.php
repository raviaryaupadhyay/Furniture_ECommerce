...
<nav class="w3-sidenav w3-collapse w3-theme-l1 w3-animate-left" style="z-index:3;width:300px;"><br>
  ...
  <div class="w3-main" style="margin-left:300px;margin-top:43px;">
   ...
   <?php
  $sql="SELECT * FROM product_upload WHERE product_status IS NULL OR product_status=0 OR product_status=1";
  $res=mysqli_query($con,$sql);
  if (mysqli_num_rows($res) > 0) {
    // output data of each row
    while($row = mysqli_fetch_assoc($res)) {
        echo '<div class="w3-container w3-quarter w3-padding">
                <div class="w3-card-4 w3-center">
                  ...
                  <a href="details.php?id='.$row["product_id"].'" class="w3-btn w3-margin w3-theme-l1">Details</a>
                </div>
              </div>';
        }
      }
